...
<Grid direction="row" class="h-full gap-10 p-4" items="stretch">
  <div class="flex flex-col w-64 h-full"> 
    ...
    <Grid direction="row">
    {#if prevSlide}
      <Card href="{base}/fsl/{page.params.esperienza}/{prevSlide.id}" bg="flex-1 bg-rose-200 text-rose-950 border-rose-950 "><span class="material-symbols-outlined">arrow_back</span></Card>
    {/if}
    {#if nextSlide}
      <Card href="{base}/fsl/{page.params.esperienza}/{nextSlide.id}" bg="flex-1 bg-rose-200 text-rose-950 border-rose-950"><span class="material-symbols-outlined">arrow_forward</span></Card>
    {/if}
    </Grid>
</div>
